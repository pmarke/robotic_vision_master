...
namespace robotic_vision {
	...
	void LK_Matcher::find_correspoinding_features(const cv::Mat& img, const cv::Mat& prev_image, std::vector<cv::Point2f>& prev_features, std::vector<cv::Point2f>& matched_features, const cv::Mat& mask){
		...
		if (display_)
			display(tempImg,matched_features);
	...
	void LK_Matcher::display(const cv::Mat& img, std::vector<cv::Point2f>& matched_features) {
		...
		for( int i = 0; i < matched_features.size(); i++) {
				...
				cv::circle(alteredImg,
				 matched_features[i],5, 
				 cv::Scalar(255,0,0), 3 );
		}

		cv::imshow("LK_Matches", alteredImg);
		cv::waitKey(1);
